...
// Plantilla ID 9 corresponde a la plantilla de receta Word (.docx)
...
export default function RecetaMedicaPage() {
  ...
  useEffect(() => {
    async function load() {
      const userEmail = getSessionCookie('user_email');
      setIsReadOnly(
        userEmail === 'administracion.nhpanamericano@gmail.com' ||
        userEmail === 'laboratorio@hospitalpanamericano.com.ec' ||
        userEmail === 'laboratorioce@hospitalpanamericano.com.ec'
      );
      try {
        const atencionData = await findOrCreateAtencion(categoriaPacienteId);
        setAtencionId(atencionData.id);

        if (atencionData.recetaMedica) {
          setInitialData((atencionData.recetaMedica.datos ?? {}) as any);
        }
  ...
  const handleGuardar = async (datosPlano: Record<string, any>) => {
    ...
    try {
      setGuardando(true);
      await upsertSeccion(atencionId, 'receta-medica', PLANTILLA_RECETA_ID, datosPlano);
      formRef.current?.clearAutosave?.();
      alert('Receta Médica guardada exitosamente.');
    } catch (err) {
      console.error(err);
      alert('Error al guardar la receta médica.');
    } finally {
      setGuardando(false);
    }
  };
  ...
  const handleExportarDocx = async (datosPlano: Record<string, any>) => {
    ...
    try {
      ...
      await exportarSeccion(PLANTILLA_RECETA_ID, 'receta-medica', datosPlano, 'docx', nombrePaciente);
      formRef.current?.clearAutosave?.();
    } catch (err) {
      console.error(err);
