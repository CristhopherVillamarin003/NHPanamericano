...
const ConsultaexternaForm: any = dynamic(
  () => import('@/components/atencion/ConsultaexternaForm'),
  { ssr: false },
);
...
export default function ConsultaPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const consultaDatos = (atencionData.consulta?.datos ?? {}) as any;
        setInitialData(consultaDatos);
      } catch (err) {
  ...
  const handleGuardar = async (datosPlano: Record<string, any>) => {
    ...
    try {
      setGuardando(true);
      await upsertSeccion(atencionId, 'consulta', 0, datosPlano);
