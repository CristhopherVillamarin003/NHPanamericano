...
const ExamenesForm: any = dynamic(
  () => import('@/components/atencion/ExamenesForm'),
  { ssr: false },
);
...
export default function ExamenesPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const examenesDatos = (atencionData.examenes?.datos ?? {}) as any;
        setInitialData(examenesDatos);
      } catch (err) {
  ...
  const handleGuardar = async (datosPlano: Record<string, any>) => {
    ...
    try {
      setGuardando(true);
      await upsertSeccion(atencionId, 'examenes', 0, datosPlano);
      formRef.current?.clearAutosave?.();
      alert('Exámenes guardados exitosamente.');
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="form-page-container">
      ...
      <div className="form-page-body" style={{ overflowY: "auto" }}>
        <ExamenesForm
          ref={formRef}
