...
const ConsentimientoForm024 = React.forwardRef<ConsentimientoFormHandle, Props>(({
  ...
  const [reverso, setReverso] = useState<DatosReverso>({
    ...
    nombre_paciente_firma:
      `${paciente?.primer_nombre ?? ""} ${paciente?.segundo_nombre ?? ""} ${paciente?.primer_apellido ?? ""} ${paciente?.segundo_apellido ?? ""}`.trim(),
    cedula_paciente_firma: paciente?.cedula ?? "",
    nombre_profesional: "",
    nombre_representante_c: "",
    cedula_representante_c: "",
    parentesco_c: "",
    fecha_negativa: "",
    nombre_paciente_negativa:
      `${paciente?.primer_nombre ?? ""} ${paciente?.segundo_nombre ?? ""} ${paciente?.primer_apellido ?? ""} ${paciente?.segundo_apellido ?? ""}`.trim(),
    cedula_paciente_negativa: paciente?.cedula ?? "",
    nombre_profesional_neg: "",
    nombre_representante_d: "",
    cedula_representante_d: "",
    parentesco_d: "",
    nombre_testigo: "",
    cedula_testigo: "",
    nombre_paciente_rev:
      `${paciente?.primer_nombre ?? ""} ${paciente?.segundo_nombre ?? ""} ${paciente?.primer_apellido ?? ""} ${paciente?.segundo_apellido ?? ""}`.trim(),
    cedula_paciente_rev: paciente?.cedula ?? "",
    nombre_profesional_rev: "",
    nombre_representante_e: "",
    cedula_representante_e: "",
    parentesco_e: "",
    ...initialData?.reverso,
  });
